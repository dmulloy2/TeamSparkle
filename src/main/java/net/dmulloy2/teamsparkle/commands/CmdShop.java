...
public class CmdShop extends PaginatedCommand
{
	...
	{
		return plugin.getShopManager().getShopList().size();
	}
	...
	public List<String> getLines(int startIndex, int endIndex) 
	{
		...
		{
			ShopItem item = plugin.getShopManager().getItem(i);
			if (item != null)
			{
				String line = FormatUtil.format(
						plugin.getMessage("shop_entry"), i,
						item.getMessage(), 1, item.getCost());
				lines.add(line);
			}
		}
